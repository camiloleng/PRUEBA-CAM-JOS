...
for p in sorted(df['periodo'].unique()):
	for x in sorted(df.loc[df['periodo']==p,'serviciosentido'].unique()):
		...
		if dd['cargacorregida_OT14'].sum()>0:
			plt.plot(dd['cargacorregida_OT14'], marker='.', markersize=3, color=ColorP[0], ls='-',  lw=1, alpha=0.5, label='Carga Corregida OT14', zorder=10)
		...
		plt.title('PERFIL DE CARGA\nSERVICIO: {} ({})   PERIODO: {}'.format(x,SERVICIO,p), fontsize='x-large')
		...
		"""
		if p==4:
			SAVEPATH = '04 FIGURAS/01 PUNTA MAÑANA/{}.png'.format(x)
		elif p==6:
			SAVEPATH = '04 FIGURAS/02 FUERA DE PUNTA/{}.png'.format(x)
		elif p==9:
			SAVEPATH = '04 FIGURAS/03 PUNTA TARDE/{}.png'.format(x)
